...
location="eastus"
sku="Standard_LRS"
subscriptionName="Free Trial" # Enter your subscription name or you can run $(az account show --query name --output tsv)
...
if [[ $? != 0 ]]; then
    echo "No [$resourceGroupName] resource group found in [$subscriptionName] subscription"
...
if [[ $? != 0 ]]; then
    echo "No [$storageAccountName] storage account found in the [$subscriptionName] subscription"
...
if [[ $? != 0 ]]; then
    echo "No [$containerName] container found in the [$storageAccountName] storage account"
